...
MODEL_PATH = os.environ.get("PORT_MODEL", os.path.join("Models", "port_count.pt"))
CONF = 0.23
BOX_W = 30
BOX_H = 35
...
def get_port_detections(img, model, conf: float = CONF):
    # agnostic_nms lets Empty_port and Connected_port detections suppress each
    # other when they overlap — without it, both classes can fire on the same
    # physical port and create duplicate boxes. iou=0.3 is stricter than the
    # default 0.7 so even slightly overlapping duplicates get culled.
    results = model(img, conf=conf, iou=0.3, agnostic_nms=True)
...
def get_boxes(cols, r1, r2, hw=None, hh=None, detections=None, img_h=None):
    ...
    # Use actual YOLO detection box dimensions when available — these are
    # direct measurements of port size, more accurate than dx-based guesses.
    if detections:
        det_widths  = [d['bbox'][2] - d['bbox'][0] for d in detections if 'bbox' in d]
        det_heights = [d['bbox'][3] - d['bbox'][1] for d in detections if 'bbox' in d]
        if det_widths and det_heights:
            hw = max(3, int(np.median(det_widths))  // 2)
            hh = max(3, int(np.median(det_heights)) // 2)

    # Cap box height to a fraction of crop height (prevent oversized boxes)
    if img_h:
        max_hh = max(5, int(img_h * 0.30))
        hh = min(hh, max_hh)
    ...
    for c in cols:
        ...
        elif t == 'separate_bot':                                # ← FIX: was "elif t == 'separate_bot' and r2:"
            if r1:                                               # ← FIX: probe top row too
                boxes.append((cx - hw, r1 - hh, cx + hw, r1 + hh))
            if r2:
                boxes.append((cx - hw, r2 - hh, cx + hw, r2 + hh))
        # (no else — any other type is intentionally skipped)

    # Snap each box's y-center to the nearest YOLO detection's actual y
    # so boxes align exactly on ports, not at row-average positions
    if detections and boxes:
        # Search radius: generous enough to find the column's detection
        search_hw = max(hw, 15)
        snapped = []
        for b in boxes:
            bcx = (b[0] + b[2]) // 2
            bcy = (b[1] + b[3]) // 2
            bh = b[3] - b[1]
            # Find detections at a similar x-column
            nearby = [d for d in detections if abs(d['center'][0] - bcx) <= search_hw]
            if nearby:
                best = min(nearby, key=lambda d: abs(d['center'][1] - bcy))
                ncy = best['center'][1]
                # Only snap if detection is within one box-height (avoid cross-row snap)
                if abs(ncy - bcy) <= bh:
                    half_h = bh // 2
                    snapped.append((b[0], ncy - half_h, b[2], ncy + half_h))
                else:
                    snapped.append(b)
            else:
                snapped.append(b)
    ...
    # Column-based validation: keep only boxes whose centre x aligns
    # with at least one YOLO detection (unchanged from original)
    if detections and boxes:
    ...
    # Remove any overlapping boxes — keep the first (higher-priority) one
    if len(boxes) >= 2:
        clean = [boxes[0]]
        for b in boxes[1:]:
            dominated = False
